...
import { BentoGrid2Demo } from "./bento-grid-2-demo";
import { bentoCard2Props, bentoGrid2Props } from "./props-data";

const BentoGridContent = () => {
  const [installTab, setInstallTab] = useState<"cli" | "manual">("cli");

  const { code: componentCode, loading: componentLoading } =
    useComponentSource("components/ui/bento-grid-2.tsx");

  const { code: previewCode, loading: previewLoading } = useComponentSource(
    "lib/blocks/content/bento-grid-2/bento-grid-2-demo.tsx"
  );
 

  const { code: usageCode, loading: usageLoading } = useComponentSource(
    "lib/blocks/content/bento-grid-2/usage.tsx"
  );

  const cliCommand = `npx shadcn@latest add https://gridmantle.vercel.app/r/bento-grid-2.json`;
    ...
    <div className="space-y-12">
      ...
      <section className="w-full">
        {previewLoading ? (
          ...
          <DemoPreview preview={<BentoGrid2Demo />} code={previewCode} />
        )}
      </section>
