...
import { TestimonialsGridDemo } from "./testimonials-grid-demo";
...
const TestimonialsGridContent = () => {
  const [installTab, setInstallTab] = useState<"cli" | "manual">("cli");

  const { code: componentCode, loading: componentLoading } = useComponentSource(
    "components/ui/testimonials-grid.tsx"
  );

  const { code: previewCode, loading: previewLoading } = useComponentSource(
    "lib/blocks/content/testimonials-grid/testimonials-grid-demo.tsx"
  );

  const { code: usageCode, loading: usageLoading } = useComponentSource(
    "lib/blocks/content/testimonials-grid/usage.tsx"
  );

  const cliCommand = `npx shadcn@latest add https://gridmantle.vercel.app/r/testimonials-grid.json`;
  const installDeps = `npm install framer-motion @remixicon/react`;

  const PROPS_TABLES = [
    {
      title: "Testimonials Grid Props",
      rows: testimonialsGridProps,
    },
    {
      title: "Testimonial Card Props",
      rows: testimonialCardProps,
    },
    {
      title: "Video Testimonial Card Props",
      rows: videoTestimonialCardProps,
    },
    {
      title: "Featured Testimonial Card Props",
      rows: featuredTestimonialCardProps,
    },
    {
      title: "Testimonial Column Props",
      rows: testimonialColumnProps,
    },
  ];

  return (
    <div className="space-y-12">
      {/* Preview Section - Same as other blocks */}
...
export default TestimonialsGridContent;
